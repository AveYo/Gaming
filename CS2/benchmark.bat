@(set "0=%~f0" ') & powershell -nop -c .([scriptblock]::create((get-content $env:0 -raw))) %* & exit /b ')
@'
...
dir "$GAME\prof_*.csv" | foreach {
  ...
  $text = (get-content $file -raw) -split "Total Frames.*|Frame Rate.*"
...
if (((get-content "$GAME\cfg\autoexec.cfg" -raw -ea 0)+'') -notlike "*$bb*") { add-content "$GAME\cfg\autoexec.cfg" $bb ; "OK" }
