@(set "0=%~f0"')& powershell -nop .([scriptblock]::create((cat -raw $env:0))) %*& exit /b ')
@'
  generate Benchmark.cfg by AveYo, 2026.05.12   run from CS2 in-game console with: BB
...
$vdf = vdf_parse (gc "$STEAMPATH\steamapps\libraryfolders.vdf" -force -ea 0)
...
## AveYo extra: print Med Mean Mode stats at the <0.1% <1% <5% =50% >90% marks
function stats($d, $l=0, $c=0) {
  ...
  if ($l -eq  0) { write-output ("Mean: {0,8}  Median: {1,8}  Mode: {2,4}" -f $m1,$m2,$m3) }
  if ($l -eq -1) { write-output ("Mean: {0,8}  Median: {1,8}  Mode: {2,4}" -f $m4,$m5,$m6) }
}
## AveYo: process the cl_showfps 4 csv files
del "$GAME\prof_*_stats.csv" ## remove files generated by the v1 script
dir "$GAME\prof_*.csv" | foreach {
  $file = $_; write-host -fore 14 -back 0 $file
  $text = (get-content -raw $file) -split "Total Frames.*|Frame Rate.*"
  ...
  $e = $enum.count; $d = $data.count; $n = 0; $total = "Total : $d`r`n"; $save += $total
...
$cfg = "" + (get-content -ea 0 -raw "$GAME\cfg\autoexec.cfg")
if ($cfg -notmatch 'alias bb "alias set v2;sv_cheats 1;exec_async benchmark"') {
  add-content "$GAME\cfg\autoexec.cfg" 'alias bb "alias set v2;sv_cheats 1;exec_async benchmark"'
}

set-content "$GAME\cfg\benchmark.cfg" @'
...
set-content "$GAME\cfg\benchmark2.cfg" @'
