...
function stats($d, $c=0) {
  ...
  write-output ("Mean: {0,-4} Median: {1,-4} Mode: {2,-4}" -f $mean,$median,$mode)
}
...
if (test-path "$GAME\$filename.csv") {
  ...
  $f2+= $list |foreach {$n = 0} {
    ...
    if ($p95 -gt 0 -and $n -ge $p95) { $o += ", >  95% $(stats $val[$p95..$k])"; $p95 = 0 }
    if ($p50 -gt 0 -and $n -ge $p50) { $o += ", =  50% $(stats $val[0..$k])";    $p50 = 0 }
